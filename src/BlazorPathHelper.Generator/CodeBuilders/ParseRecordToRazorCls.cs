    ...
    INamedTypeSymbol rootSymbol)
{
    ...
    public string ExportInheritClassCode()
    {
        // Handle the @inherits directive
        var pageTemplate = rootSymbol.GetAttributes()
            .FirstOrDefault(a => a.AttributeClass?.Name == nameof(BlazorPathAttribute))
            ?.NamedArguments
            .FirstOrDefault(arg => arg.Key == nameof(BlazorPathAttribute.PageTemplate))
            .Value.Value as INamedTypeSymbol;

        var inheritsType = record.IsInheritedOverride ? record.InheritTypeSymbol : pageTemplate;
        return inheritsType != null ? $" : {inheritsType.ToDisplayString()}" : "";
    }
