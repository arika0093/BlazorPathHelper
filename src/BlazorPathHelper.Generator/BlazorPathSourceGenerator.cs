...
public class BlazorPathHelperSourceGenerator : IIncrementalGenerator
{
        ...
        (GeneratorAttributeSyntaxContext source, ImmutableArray<ParseRazorStructure> structures) pairs)
    {
        ...
        ExportRazorClassCode(context, parseRecords, structures, symbol);
    }
        ...
        INamedTypeSymbol rootSymbol)
    {
        ...
        foreach (var record in targetRecords)
        {
            ...
            var inheritsType = record.Inherits ?? pageTemplate;
            if (inheritsType != null)
            {
                inheritsDirective = $" : {inheritsType.ToDisplayString()}";
            }
            ...
            {{record.AccessModifier}} partial class {{pageClassName}}{{inheritsDirective}}
            {
