...
public class BlazorPathHelperSourceGenerator : IIncrementalGenerator
{
    ...
    private static void EmitPathRoot(SourceProductionContext context, GeneratorAttributeSyntaxContext source)
    {
        ...
        var builders = fields
            .Select(field =>
            {
                // if it has [BlazorPathItem] attribute, use it
        ...
        var code = FinallyBuildCode(builders, symbol, accessibility, namespaceName, exportClassName);
        ExportCodeFile(context, symbol, code);
        ExportBlazorMenuCode(builders.ToArray(), context, symbol, accessibility, namespaceName, exportClassName);
    }

    private static string FinallyBuildCode(
        IEnumerable<PathAttributeParser> builders, ISymbol symbol,
        string accessibility, string namespaceName, string exportClassName)
    {
        var code = $$"""
         // <auto-generated />
         #nullable enable
         ...
         {{accessibility}} partial class {{exportClassName}}
         {
             /// <summary>
    ...
    private class PathAttributeParser
    {
        ...
        internal string? MenuIconBuilder =>
            PathItemAttr?.CustomIcon != null ? $"new {PathItemAttr.CustomIcon}()" :
            PathItemAttr?.Icon != null ? $"\"{PathItemAttr?.Icon}\"" :
            null;
        ...
        // try format /sample/{value}/{val2} -> /sample/{0}/{1}
    ...
    private static T MapToType<T>(AttributeData attributeData) where T : Attribute
    {
        ...
        foreach (var p in attributeData.NamedArguments)
        {
            ...
            if (property.PropertyType == typeof(Type) && value is INamedTypeSymbol namedTypeSymbol)
            {
                // Type is a special case, cannot get it directly from the value
                continue;
            }
