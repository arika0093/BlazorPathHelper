Certainly! Here's the translated text into naturalistic English:
...
// By adding the BlazorPath attribute, automatic generation occurs.
// Also, the class definition must include the partial attribute.
[BlazorPath]
...
A URL builder that accepts an `#!csharp int` type parameter has been generated for `WebPaths.Helper.CounterWithState`. However, `CounterWithQuery` is not yet set up to accept queries. This is because the queries it will accept have not been defined yet.
...
// It is recommended to specify default values or make each parameter nullable.
public record QueryRecord(string query = "hello", int page = 0, bool? opt = null);
```

!!! warning "Note!"

    The class definition for `QueryRecord` above will not be generated correctly if written in a `.razor` file. This is due to the nature of source generators. (Razor files are also converted to C# by source generators, causing conflicts.)
...
On the Blazor page side, attributes like `#!razor @page`, `#!csharp [Parameter]`, and `#!csharp [SupplyParameterFromQuery]` need to be specified, but these can also be automatically generated based on URL definition information.
...
public record QueryRecord(string query = "hello", int page = 0, bool? opt = null);
// Definition of each component (actually written in each component)
...
Then, a file containing class definitions with `#!razor @page`, `#!csharp [Parameter]`, and `#!csharp [SupplyParameterFromQuery]` is **automatically** generated.
...
With this, page attributes are automatically set, eliminating the need to write them in `.razor` files. Therefore, you can remove the `@page` attribute and others from `.razor` files. For detailed usage, refer to [Automatic Page Attributes](../Features/AutoPageAttribute/index.md).

!!! tip "You Can Remove It Anytime"

    The automatic generation by the `#!csharp [Page]` attribute is a completely independent feature. Therefore, if automatic definitions are unnecessary, you can remove the `[Page]` attribute at any time.

## Automatic Menu Structure Generation
Based on URL definitions, menu structure data is also automatically generated. This is useful for dynamically creating menus.
...
public partial class WebPaths
{
  // By adding the [Item("MenuName")] attribute, automatic generation occurs.
...
Then, `WebPaths.MenuItem` automatically generates menu structure data.
    ...
    public partial class WebPaths
    {
      public static readonly BlazorPathMenuItem[] MenuItem = [
        new BlazorPathMenuItem(){
          ...
          Path = "/",       // Page URL
          Children = []     // Submenu
          // There are several other properties automatically generated for convenient menu creation.
        },
        new BlazorPathMenuItem(){
          Name = "Sample1a",
          Path = "/sample", 
...
Here, only the menu structure data is generated, and you need to implement the menu display separately. In other words, it's up to you which framework you use to create the menu!
...
### Customization
During menu generation, customization such as descriptions, icons, and localization is possible. For details, refer to [Menu Structure Customization](../Features/MenuBuilder/MenuCustomization.md).